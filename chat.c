...
//set nickname
printf("Please enter your nickname(Max to 10): ");
fgets(kb_msg, nickname_size+1,stdin);
sprintf(nickname,"%s",kb_msg);
for(i=0;i<=nickname_size+1;i++)
{
	if(nickname[i]=='\n')
		nickname[i]='\0';
}
write(sockfd,nickname,strlen(msg));
//
...
msg[result]='\0';
strcat(kb_msg,msg+1);*/
sprintf(msg, "M<%s> : %s",nickname, kb_msg);
write(sockfd, msg, strlen(msg));
}
}
}
